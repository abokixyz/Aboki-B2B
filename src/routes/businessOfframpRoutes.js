...
/*
BUSINESS OFF-RAMP API ROUTES SUMMARY

Public Endpoints (Require API Key):
├── POST   /quote                           - Get off-ramp quote
├── POST   /create                          - Create off-ramp order
├── GET    /orders                          - List all orders
├── GET    /orders/:orderId                 - Get specific order
├── GET    /stats                           - Business statistics
├── POST   /orders/:orderId/cancel          - Cancel order
├── POST   /orders/:orderId/retry           - Retry failed order
├── GET    /banks                           - List supported banks
├── POST   /verify-account                  - Verify bank account
├── GET    /supported-tokens                - List supported tokens
├── GET    /token-info/:network/:address    - Get token details
├── GET    /analytics/volume                - Volume analytics
├── GET    /health                          - Service health check
└── GET    /config                          - Service configuration

Webhook Endpoints (No Authentication):
├── POST   /webhook/deposit-confirmation    - Deposit webhooks
└── POST   /webhook/payout-status          - Payout webhooks

Internal Endpoints (Require Internal API Key):
├── POST   /internal/monitor/expired-orders - Monitor expired orders
├── POST   /internal/monitor/stuck-orders   - Monitor stuck orders
└── GET    /internal/stats/system           - System statistics

Features:
✅ Token-to-NGN conversion with multiple networks
✅ Automatic wallet generation for deposits
✅ Bank account verification via Lenco API
✅ Real-time webhooks for order updates
...
- Internal API key for system endpoints
...
- 25+ supported banks via Lenco API
- Real-time account name resolution
- Account format validation

Processing Flow:
1. Customer gets quote → 2. Business creates order → 3. Auto wallet generated
4. Customer deposits tokens → 5. Deposit detected → 6. Tokens swapped to USDC
7. NGN payout to bank → 8. Order completed → 9. Webhooks sent

Monitoring & Analytics:
- Real-time order tracking
- Volume analytics with grouping
